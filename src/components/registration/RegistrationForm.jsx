...
function RegistrationForm() {
  ...
  const getInputstyles = (condition) => {
    const styles = {
      borderColor: condition ? "#FF0000" : "transparent",
    };
    return styles;
  };
  const nameinputStyle = getInputstyles(nameErr);
  const usernameinputStyle = getInputstyles(usernameErr);
  const mailinputStyle = getInputstyles(mailErr);
  const mobileinputStyle = getInputstyles(mobileErr);
  return (
